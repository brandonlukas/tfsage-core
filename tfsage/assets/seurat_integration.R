...
#   metadata in h5 and Parquet formats as input, respectively, preprocesses
#   the data, and optionally performs integration. The embeddings are saved in
#   Parquet format for each specified method.
#
# Usage:
...
#   - Input files in h5 and Parquet formats.
# ------------------------------------------------------------------------------
...
option_list <- list(
  make_option(c("--rp-matrix"),
    type = "character",
    help = "Path to RP matrix file (input, h5 format).",
    metavar = "file"
  ),
...
seurat_object <- CreateSeuratObject(
  counts = Read10X_h5(filename = args$`rp-matrix`),
