...
def process_bed_file(
    bed_file: str, decay_factor: float = 10_000, dtype: np.dtype = np.float32
) -> np.ndarray:
    features = _extract_features(bed_file, _gene_loc_set, decay_factor, dtype=dtype)
    return features
...
def extract_features_parallel(
    ...
    with ProcessPoolExecutor(
        ...
        futures = {
            executor.submit(process_bed_file, bed_file, decay_factor, dtype): i
