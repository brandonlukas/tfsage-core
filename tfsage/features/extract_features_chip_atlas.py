...
def _extract_features_ripgrep(
    ...
    features = _extract_features(stdout.name, _gene_loc_set, decay_factor, dtype=dtype)
    return features
...
def process_bed_file_for_name(
    ...
    if data_path and os.path.exists(data_path):
        ...
        with (
            ...
            features = _extract_features_ripgrep(
                bed_file, name, stdout, decay_factor, dtype=dtype
            )
    return features
...
def extract_features_chip_atlas(
    ...
    Parameters:
        ...
        dtype (np.dtype): Data type for the features. Default is np.float32.
    ...
    with ProcessPoolExecutor(
        ...
        futures = {
            executor.submit(
                process_bed_file_for_name, bed_file, name, decay_factor, data_dir, dtype
            ): i
